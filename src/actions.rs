    ...
    if output.update_available {
        ...
        if let Ok(output_file) = env::var("GITHUB_OUTPUT") {
            ...
            fs::write(&output_file, content)
                .with_context(|| format!("Failed to write to GITHUB_OUTPUT file: {}", output_file))?;
            log::debug!("Wrote output to GITHUB_OUTPUT file: {}", output_file);
        }

        log::info!("Set GitHub Actions output: update available, version={}, checksum={}",
...
pub fn log_release_decision(should_create: bool, reason: &str, version: &str) {
    if should_create {
        ...
        log::info!("Release will be created by GitHub Actions for version {}", version);
    } else {
